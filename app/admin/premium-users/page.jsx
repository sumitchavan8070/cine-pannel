...
export default function PremiumUsersPage() {
  ...
  const handleAssignPremium = async () => {
    ...
    try {
      const token = getAdminToken()
      // Always assign yearly plan (365 days) - API will automatically use yearly plan
      await apiPost(`/admin/users/${selectedUser.id}/assign-premium`, {
        plan_id: null, // Let API determine the yearly plan
      }, { token })
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <div className="p-3 bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-lg">
                  <p className="text-sm font-semibold text-yellow-800 mb-1">Yearly Premium Plan</p>
                  <p className="text-xs text-yellow-700">
                    User will be assigned a yearly premium subscription (365 days) that starts today.
                  </p>
                </div>
              </div>
            </div>
